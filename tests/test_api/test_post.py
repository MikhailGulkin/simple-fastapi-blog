...
async def test_create_post(
    client: AsyncClient, user_data, create_user_in_database
) -> None:
    await create_user_in_database(**user_data)
    ...
    response = await client.post("/post/create-post", json=data_json)
...
async def test_update_post(
    ...
    response = await client.patch(
        f'/post/update-post/{post_data["post_id"]}', json=data_json
    )
    json = response.json()
