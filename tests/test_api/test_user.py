...
async def test_get_all_users(
    client: AsyncClient, create_user_in_database, user_data
) -> None:
...
async def test_create_user(
    ...
    data_json = {"username": "123", "email": "123@mail.com", "password": "123"}
    response = await client.post("/user/create-user", json=data_json)
...
async def test_update_user(
    ...
    response = await client.patch(
        f'/user/update-user/{user_data["user_id"]}', json=update_data
    )
    json = response.json()
